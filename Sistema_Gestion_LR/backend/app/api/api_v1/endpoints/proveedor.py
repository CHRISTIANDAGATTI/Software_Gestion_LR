...
from app.schemas.proveedor import Proveedor, ProveedorBase
...
def crear_proveedor(proveedor_in: ProveedorBase, db: Session = Depends(get_db)):
    return crud.proveedor.create(db, proveedor_in)

@router.put("/proveedores/{id}", response_model=Proveedor)
def actualizar_proveedor(id: int, proveedor_in: ProveedorBase, db: Session = Depends(get_db)):
